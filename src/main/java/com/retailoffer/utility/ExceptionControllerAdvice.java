...
public class ExceptionControllerAdvice {
	...
	public ResponseEntity<ErrorInfo> handleMethodArgumentNotValidException(MethodArgumentNotValidException exception) {
		...
		String messageKey = exception.getBindingResult().getFieldErrors().stream().findFirst()
				.map(fieldError -> fieldError.getDefaultMessage()).orElse("validation.default");
	...
	public ResponseEntity<ErrorInfo> handleConstraintViolationException(ConstraintViolationException exception) {
		...
		String messageKey = exception.getConstraintViolations().stream().findFirst()
				.map(violation -> violation.getMessage()).orElse("constraint.violation");
	...
	public ResponseEntity<ErrorInfo> handleGenericException(Exception exception) {
		...
		String errorMessage = environment.getProperty("general.exception",
